...
The context itself — `OverlayHostContext` — exposes three methods: `registerLayer`, `unregisterLayer`, and `updateLayer`. When an exit transition finishes, `LayerRenderer` calls `unregisterLayer` directly to remove the layer. The context value is memoized so that its identity is stable across re-renders as long as the callback identities do not change, which prevents unnecessary re-renders of consumers that read the context. If a component tries to use the context outside of an `<OverlayHost>`, the `useOverlayHost` hook throws an error immediately, making misconfiguration loud and obvious.
...
There is one subtlety worth noting. `<Layer>` splits its lifecycle across three effects. The first is an open/close transition effect keyed on `[isOpen]` only: it registers on a false→true edge, tears down (immediately or via an exit transition) on a true→false edge, and re-registers under React.StrictMode's dev unmount-remount cycle. The second is a dedicated structural props-update effect that pushes shallow-merged patches of the structural props (z, capture, backdrop, anchor, explicitPosition, transition, etc.) to the host via `updateLayer` whenever those props change; it skips its initial run because the open/close effect already pushes the full descriptor on first open. Neither of these effects depends on `children`, so a content-only change would leave the host's descriptor stale. To close that gap, `<Layer>` runs a third effect after every render (no dependency array) that compares the current children ref against a previously-synced snapshot; if they differ, it pushes a content-only update to the host via `updateLayer`.
