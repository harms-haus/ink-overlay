...
describe('anchorToFlexbox', () => {
	const cases: Array<{
		...
		{anchor: 'top-left', alignItems: 'flex-start', justifyContent: 'flex-start'},
		{anchor: 'top-right', alignItems: 'flex-start', justifyContent: 'flex-end'},
		{anchor: 'bottom-left', alignItems: 'flex-end', justifyContent: 'flex-start'},
		{anchor: 'bottom-right', alignItems: 'flex-end', justifyContent: 'flex-end'},
	];
...
describe('computeAnchorCoords', () => {
	...
	test('center: top=floor((rows-height)/2), left=floor((columns-width)/2)', () => {
		...
		expect(computeAnchorCoords('center', viewport, layer)).toEqual({top: 9, left: 30});
	});

	test('top: top=0, left=centered', () => {
		// Top=0, left=floor((80-20)/2)=30
		expect(computeAnchorCoords('top', viewport, layer)).toEqual({top: 0, left: 30});
	});

	test('bottom: top=rows-height, left=centered', () => {
		// Top=24-6=18, left=30
		expect(computeAnchorCoords('bottom', viewport, layer)).toEqual({top: 18, left: 30});
	});

	test('left: left=0, top=centered', () => {
		// Left=0, top=9
		expect(computeAnchorCoords('left', viewport, layer)).toEqual({top: 9, left: 0});
	});

	test('right: left=columns-width, top=centered', () => {
		// Left=80-20=60, top=9
		expect(computeAnchorCoords('right', viewport, layer)).toEqual({top: 9, left: 60});
	});

	test('top-left: top=0, left=0', () => {
		expect(computeAnchorCoords('top-left', viewport, layer)).toEqual({top: 0, left: 0});
	});

	test('top-right: top=0, left=columns-width', () => {
		expect(computeAnchorCoords('top-right', viewport, layer)).toEqual({top: 0, left: 60});
	});

	test('bottom-left: top=rows-height, left=0', () => {
		expect(computeAnchorCoords('bottom-left', viewport, layer)).toEqual({top: 18, left: 0});
	});

	test('bottom-right: top=rows-height, left=columns-width', () => {
		expect(computeAnchorCoords('bottom-right', viewport, layer)).toEqual({top: 18, left: 60});
	});

	test('margin offsets are applied and clamped to >= 0', () => {
		const result = computeAnchorCoords('top-left', viewport, layer, {top: 2, left: 3});
		expect(result).toEqual({top: 2, left: 3});
	});

	test('margin cannot produce negative values (clamped to 0)', () => {
		const result = computeAnchorCoords('center', viewport, layer, {top: -50, left: -50});
...
describe('computePopoverPosition', () => {
	...
	test('placement \'bottom\': popover appears below the anchor', () => {
		const anchorRect = {
			left: 30, top: 5, width: 10, height: 2,
		};
		...
		const result = computePopoverPosition(anchorRect, popover, viewport, 'bottom');
	...
	test('placement \'bottom-start\': left aligned to anchor left', () => {
		...
		const result = computePopoverPosition(anchorRect, popover, viewport, 'bottom-start');
		expect(result.top).toBe(11);
	...
	test('placement \'bottom-end\': left aligned to anchor right edge minus popover width', () => {
		...
		const result = computePopoverPosition(anchorRect, popover, viewport, 'bottom-end');
		expect(result.top).toBe(11);
	...
	test('placement \'top\': popover appears above the anchor', () => {
		const anchorRect = {
			left: 30, top: 15, width: 10, height: 2,
		};
	...
	test('placement \'right\': popover appears to the right of the anchor', () => {
		const anchorRect = {
			left: 10, top: 10, width: 5, height: 2,
		};
		...
		const result = computePopoverPosition(anchorRect, popover, viewport, 'right');
	...
	test('placement \'left\': popover appears to the left of the anchor', () => {
		const anchorRect = {
			left: 40, top: 10, width: 5, height: 2,
		};
		// Left: left = 40 - 4 - 20 = 16
		// top (center) = 10 + (2-5)/2 = 8.5, floor → 8
		const result = computePopoverPosition(anchorRect, popover, viewport, 'left');
	...
	test('custom offset is used', () => {
		...
		const result = computePopoverPosition(anchorRect, popover, viewport, 'bottom', {offset: 8});
	...
	test('flip: near-bottom anchor flips to top', () => {
		...
		const anchorRect = {
			left: 30, top: 20, width: 10, height: 2,
		};
	...
	test('flip: near-top anchor with placement top flips to bottom', () => {
		const anchorRect = {
			left: 30, top: 0, width: 10, height: 2,
		};
	...
	test('flip: near-right anchor with right placement flips to left', () => {
		const anchorRect = {
			left: 65, top: 10, width: 5, height: 2,
		};
...
describe('compareLayers', () => {
	...
	test('higher z sorts last', () => {
		expect(compareLayers({z: 5, order: 0}, {z: 2, order: 0})).toBeGreaterThan(0);
	});
	...
	test('equal z: higher order sorts last', () => {
		expect(compareLayers({z: 3, order: 5}, {z: 3, order: 2})).toBeGreaterThan(0);
	});
